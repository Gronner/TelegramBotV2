...
class Helper:
    ...
    def __init__(self, bot_dispatcher, groups=[0]):
        self._groups = groups
        self._help_string = self._build_help_string(bot_dispatcher)
    ...
    def _build_help_string(self, bot_dispatcher):
        """
        Builds the help string from the command string and the functions
        docstring.
        """
        command_handlers = self._extract_command_handlers(bot_dispatcher,
                                                          CommandHandler)
        command_help_mapping = self._map_commands_to_docstring(command_handlers)
        help_string = self._create_help_string_from_map(command_help_mapping)
        return help_string


    def _extract_command_handlers(self, dispatcher, base_type):
        """
    ...
    def _map_commands_to_docstring(self, command_handlers):
        """
    ...
    def _create_help_string_from_map(self, mapping):
        """
